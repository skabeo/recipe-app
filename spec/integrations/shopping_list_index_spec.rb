...
RSpec.feature 'User recipe index', type: :feature do
  before(:each) do
    ...
    @recipe_food_1 = RecipeFood.create(quantity: 10, recipe_id: @recipe.id, food_id: @food.id)
    @recipe_food_2 = RecipeFood.create(quantity: 30, recipe_id: @recipe.id, food_id: @food.id)
  end
  ...
  scenario 'should display recipe food quantity' do
    ...
    expect(page).to have_content(@recipe_food_1.quantity)
    expect(page).to have_content(@recipe_food_2.quantity)
  end
  ...
  scenario 'should display recipe food price' do
    ...
    visit shopping_list_index_path(recipe_id: @recipe.id)
    price = @food.price
    quantity = @recipe_food_1.quantity
